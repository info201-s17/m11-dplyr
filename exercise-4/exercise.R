...
mean(flights$time.gained, na.rm = TRUE) # answer is: -5.659779

# Create a data.frame that is of flights headed to seatac ('SEA'), 
to.sea <- flights %>% select(time.gained, dest) %>% filter(dest == 'SEA') 

# On average, did flights to seatac gain or loose time?
mean(to.sea$time.gained, na.rm = TRUE) # answer is -11.6991
